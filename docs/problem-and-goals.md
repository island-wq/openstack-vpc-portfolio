...
기존 프라이빗 클라우드 VPC는 전용 방화벽의 논리 인스턴스를 테넌트 경계로 사용했습니다. 이 방식은 명확한 격리와 익숙한 운영 모델을 제공했지만, 고객과 VPC가 증가할수록 장비·라이선스 비용도 함께 증가했습니다.

### 비용 구조

- VPC별 네트워크 장비 라이선스가 필요했습니다.
- 신규 고객에게 고정 네트워크 기본 비용이 발생했습니다.
- 랙과 테넌트 규모가 커질수록 장비 중심 비용이 누적됐습니다.

### 확장성

- 1계정 1VPC 구조로 여러 워크로드를 분리하기 어려웠습니다.
- VLAN 기반 사용자 네트워크는 식별자 수에 구조적 한계가 있었습니다.
- 기존 제어 노드가 North-South 트래픽까지 담당하면 확장 시 병목 위험이 있었습니다.

### 상품 경쟁력

다른 CSP는 VPC를 기본 격리 단위로 제공하면서 Public/Private Subnet, Route Table, Internet/NAT Gateway, Security Group과 Multi-AZ를 일관된 사용자 경험으로 제공합니다. 기존 구조에서도 유사한 기능은 가능했지만, OpenStack 자원과 장비 설정이 분리되어 상품 확장과 자동화가 어려웠습니다.
